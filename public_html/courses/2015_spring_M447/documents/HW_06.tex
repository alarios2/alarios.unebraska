...
\item[\bf Problem 1]  Let $i=\sqrt{-1}$.  Show that, for any integers $m$ and $n$, 
\begin{align*}
 \int_0^{2\pi} e^{inx}e^{-imx}\,dx &= 
 \begin{cases}
  2\pi &\text{ if } m = n,\\
  0 &\text{ if } m \neq n,\\
 \end{cases}
...
% \\
%  \int_0^{2\pi} \sin(0x)\sin(0x)\,dx &= 0.
\end{align*}
\textbf{Time saver:} Do the first two (use a trig identity on the second one), then expand the exponents in the first one with Euler's formula, ($e^{i\theta}=\cos(\theta)+i\sin(\theta)$), to get the rest.

\item[\bf Problem 2]
Consider the ``Continuous to Discrete'' Fourier transform, given by the relations
\begin{align}\label{fourier_expansion}
 f(x) = a_0 + \sum_{k=1}^\infty( a_k\cos(kx) + b_k\sin(kx))
\end{align}
\begin{enumerate}[(a)]
\item 
Find the Fourier coefficients $a_0$, $a_k$, and $b_k$ in the case where $f(x) = x$.  (Hint: Use the relationships in Problem 1 on \eqref{fourier_expansion} to isolate the coefficents.  Then integrate by parts.)  The coefficients will be numbers that depend only on $k$.  

\item Plot the first few terms of the series in \eqref{fourier_expansion} using the coefficients you found.  (Hint: It is easy to do this in Matlab using a loop over $k$ to add all the terms together.)
\end{enumerate}


\item[\bf Problem 3] Show that the quadrature 
$$
...
\item[\bf Problem 4] Find the nodes and the coefficients of the Gauss 
