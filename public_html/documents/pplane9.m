...
if strcmp(action,'initialize')
  ...
  ud.color = color;
  ud.level = ' ';
  ppset = figure('name','pplane9 Setup',...
	...
	otherwise
	 ...
	 set(gcf,'defaultuicontrolfontsize',fs*0.9);
       ...
       mequit = uimenu(fmenu,'label','Quit pplane9',...
		       'call','pplane9(''quit'')','separator','off');
	...
	ode113ch = 'off';
	 ...
	 ode23tbch = 'off';
       ...
       menu(6) = uimenu(menu(1),'label','Make the Display Window inactive.',...
			'call','pplane9(''hotcold'')','separator','on');
...
elseif strcmp(action,'linear')
   ...
   if (~isempty(pplin))
      ...
      mequit = uimenu(fmenu,'label','Quit pplane9',...
	  'call','pplane9(''quit'')','separator','on','pos',9);
	...
	chkd = 'on';
      ...
      set(pplin.Number,'WindowButtonDownFcn',['pplane9(''down'',',num2str(pplin.Number),')']);
...
elseif strcmp(action,'kbd')
   ...
   kbd.comp = uicontrol('style','push',...		
			'string','Compute','call','pplane9(''kcompute'')');
...
elseif strcmp(action,'plotxyfig')  % Build the graph figure.
  ...
  ppxy = figure('tag','pplane9','visible','off');
  name = ['pplane9 t-plot # ',int2str(ppxy.Number)];
  ud.color = dud.color;
  ud.type = 0;
