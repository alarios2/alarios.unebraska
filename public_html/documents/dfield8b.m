function output = dfield8b(action,input1,input2,input3)

% dfield8b	is an interactive tool for studying single first order
%	differential equations.  When dfield8b is executed, a dfield8b Setup
...
%	mouse button is depressed in the dfield8b Display window, the
...
%	and displayed.
%
%   Version 8b denotes changes incorporating fixes for Matlab R2014b.
...
%  Last modified: April 2, 2003
  ...
  if strcmp(action,'initialize')
    ...
    % First we make sure that there is no other copy of dfield8b
    % running, since this causes problems.	
    
    dfh = findobj('tag','dfield8b');
    if ~isempty(dfh);
      qstring = {'There are some open dfield8b figures (perhaps invisible).  ';...
		 'What do you want to do?'};
      tstring = 'Only one copy of dfield8b can be open at one time.';
      b1str = 'Restart dfield8b.';
    ...
    % Next we look for old files created by dfield8b.
	...
	ll = fgetl(fid);
	fclose(fid);
	if strcmp(ll,'%% Created by dfield8b')
	  delete(fn)
	else
	  kk = [kk;k];
	end
    ...
    dfset = figure('name','dfield8b Setup','NumberTitle','off',...
		   'tag','dfield8b','visible','off',...
		   'UserData',ud);
    
    dfield8b('figdefault',dfset);
    ...
    butt(2) = uicontrol('style','push',...
			'pos',rwind,...
			'string','Revert',...
			'call','dfield8b(''revert'')',...
			'visible','off');
  
    butt(3) = uicontrol('style','push',...
			'pos',pwind,...
			'string','Proceed',...
			'call','dfield8b(''proceed'')',...
			'visible','off');
  ...
  meproceed = uimenu(mefile,'label','Proceed','call',...
    ...
    meadd = uimenu(sysmenu,'label','Add current equation to the gallery',...
		   'call','dfield8b(''addgall'');','tag','add system');
    sep = 'on';
  ...
  elseif strcmp(action,'proceed')
    ...
    dfset = gcf;
    sud = get(dfset,'UserData');
    sud.o = sud.c;
    set(dfset,'UserData',sud);
    ...
    if any(isnan(WINvect))
      errmsg = ['One of the entries defining the display window ',...
		'is not a number.'];
      fprintf('\a')
      errordlg(errmsg,'dfield8b error','on');
      return
    end
    xstr = sud.c.xname;
    if isempty(xstr)
    ...
    if (sud.flag == 1)
      dfdisp = findobj('name','dfield8b Display');
      dud = get(dfdisp,'UserData');
	...
	set(dud.axes,'UserData',aud);
      ...
      set(dfdisp,'UserData',dud);
      dfield8b('dirfield',dfdisp);
	...
	if ~isempty(pn)
	  if isempty(pv) 
	    ...
	    xderivstr = dfield8b('paraeval',pn,pv,derivstr);
	...
	return
      ...
      % Find dfield8b Display if it exists.	
      % If dfield8b Display exists, update it.  If it does not build it.
	...
	dfdisp = figure('name','dfield8b Display',...
			'NumberTitle','off','interrupt','on',...
			'visible','off','tag','dfield8b');
	dfield8b('figdefault',dfdisp);
	dud = get(dfdisp,'UserData');	
	dud.syst = sud.c;
	dud.settings = sud.settings;
	fs = dud.fontsize;
	...
	merestart = uimenu(fmenu,'label','Restart dfield8b','call',...
			   'dfield8b(''restart'')','separator','on','pos',8);
	
	mequit = uimenu(fmenu,'label','Quit dfield8b',...
			'call','dfield8b(''quit'')','separator','on');
	...
	menu(2) = uimenu(emenu,'label','Zoom in.',...
			 'call','dfield8b(''zoomin'')',...
			 'pos',1);
	
	zbmenu = uimenu(emenu,'label','Zoom back.',...
			'call','dfield8b(''zoomback'')',...
			'enable','off',...
			'tag','zbmenu',...
			'pos',2);
	...
	solverstr = ['ud = get(gcf,''UserData'');',...
		     ...
		     'set(me,''checked'',''on'');',...
		     'set(gcf,''UserData'',ud);',...
		     'dfset = findobj(''name'',''dfield8b Setup'');',...
		     'sud = get(dfset,''UserData'');',...
	...
	dud.solver(2) = uimenu(mesolve,'label','Euler',...
			       'checked',eulch,...
			       'call',solverstr,'UserData',eulset);
	
	dud.solver(3) = uimenu(mesolve,'label','Runge-Kutta 2',...
			       'checked',rk2ch,...
			       'call',solverstr,'UserData',rk2set);
	...
	meset   = uimenu(menu(1),'label','Window settings.',...
			 'call','dfield8b(''settings'')');
	
	menu(6) = uimenu(menu(1),'label','Make the Display Window inactive.',...
			 'call','dfield8b(''hotcold'')','separator','on');
	
	% View menu
	
	set(findobj(gcf,'label','&View'),'pos',4);
	set(findobj(gcf,'label','&Figure Toolbar'),...
	    'call','dfield8b(''showbar'')');
	dud.menu = menu;
	
	set(dfdisp,'ToolBar','none');
	set(0,'showhiddenhandles',hhsetup);
	set(gcf,'WindowButtonDownFcn','dfield8b(''down'')');
	set(dfdisp,'WindowButtonMotionFcn','dfield8b(''cdisp'')');
	...
	hmax = DY(1)/10;
	dud.settings.hmax = hmax;
	mud = get(dud.solver(1),'UserData');
	mud.hmax = hmax;
	set(dud.solver(1),'UserData',mud);
	set(dfdisp,'UserData',dud);
	set(dud.axes,'UserData',ud);
	ppkbd = findobj('name','dfield8b Keyboard input','vis','on');
	if ~isempty(ppkbd),dfield8b('kbd'),end
	dfield8b('dirfield',dfdisp);	
    end
  ...
  elseif strcmp(action,'dirfield')
    ...
    % Find dfield8b Display and get the user data.
  ...
  elseif strcmp(action,'hotcold')
    ...
    if (findstr(get(mehc,'label'),'inactive'))
      ...
      set(dfdisp,'WindowButtonDownFcn','dfield8b(''down'')');
  ...
  elseif strcmp(action,'down')
    ...
    initpt = initpt(1,[1,2]);
    dfield8b('solution',initpt,disph);
  ...
  elseif strcmp(action,'several')
    ...
    %  set([ch;mh],'enable','on');
    set(ch,'enable','on');
    set(disph,'WindowbuttonDownFcn','dfield8b(''down'')');
  ...
  elseif strcmp(action,'kcompute')
    ...
    % button on the dfield8b Keyboard figure.
	...
	if isempty(pv) 
	  ...
	  xstr = dfield8b('paraeval',pn,pv,xstr);
	  tstr = dfield8b('paraeval',pn,pv,tstr);
	end
      end
    end
    xvalue = str2num(xstr);
    ...
    if compute
      dfield8b('solution',initpt,dfdisp);
    end
    nstr = get(dud.notice,'string');
    nstr(1:4) = nstr(2:5);
    nstr{5} = 'Ready.';
  ...
  elseif strcmp(action,'kbd')
    ...
    % sets up the dfield8b Keyboard figure which allows accurate input of 
    % initial values using the keyboard.
    
    ppkbd = findobj('name','dfield8b Keyboard input');
    if ~isempty(ppkbd)
      delete(ppkbd);
    end    
    dfdisp = gcf;
    ...
    ppkbd = figure('name','dfield8b Keyboard input',...
		   'vis','off',...
		   'NumberTitle','off','tag','dfield8b');
    
    dfield8b('figdefault',ppkbd);
    ...
    kbd.comp = uicontrol('style','push',...		
			 'string','Compute','call','dfield8b(''kcompute'')');
  ...
  elseif strcmp(action,'zoomin')
    ...
    set(gcf,'WindowButtonDownFcn','dfield8b(''zoom'')',...
  ...
  elseif strcmp(action,'solvset')
    ...
    setfig = findobj('name','dfield8b Solver settings');
    if ~isempty(setfig)
      data = get(setfig,'UserData');
    ...
    nstepcall =['data = get(gcf,''UserData'');',...
    ...
    setfig = figure('name','dfield8b Solver settings','NumberTitle','off',...
		    'tag','dfield8b','vis','off');
    
    dfield8b('figdefault',setfig);
    ...
    hmaxcall = ['data = get(gcf,''UserData'');',...
  ...
  elseif strcmp(action,'settings')
    ...
    % up the dfield8b Settings window, which allows the user to interactively
    ...
    setfig = findobj('name','dfield8b Window settings');
    if ~isempty(setfig)
      close(setfig);
    end
    setfig = figure('name','dfield8b Window settings',...
		    'NumberTitle','off',...
		    'tag','dfield8b','vis','off');
    ...
    for i=1:3
      set(data.rad(i),'UserData',data.rad(:,[1:(i-1),(i+1):3]));
    end
  ...
  elseif strcmp(action,'setchange')
    ...
    % on the dfield8b Settings window.
    
    dfsett = gcf;
    data = get(dfsett,'UserData');
    dfdisp = findobj('name','dfield8b Display');
    dfset = findobj('name','dfield8b Setup');
  ...
  elseif strcmp(action,'print')
    ...
    nstr = get(dud.notice,'string');
    nstr(1:4) = nstr(2:5);
    nstr{5} = 'Preparing to print the dfield8b Display Window.  Please be patient.';
    set(dud.notice,'string',nstr);
    
    nstr(1:4) = nstr(2:5);
    nstr{5} = 'Printing the dfield8b Display Window.';
    set(dud.notice,'string',nstr);
  ...
  elseif strcmp(action,'level')
    ...
    xname(find(abs(xname)==92))=[];
    dflevel = findobj('name','dfield8b Level sets');
    if ~isempty(dflevel)
      delete(dflevel)
    end
    dflevel = figure('name','dfield8b Level sets',...
		     'vis','off',...
		     'NumberTitle','off','tag','dfield8b');
    
    dfield8b('figdefault',dflevel);
    hhsetup = get(0,'showhiddenhandles');
    ...
    set(dflevel,'vis','on','UserData',lev);
    set(child,'vis','on');
    
  elseif strcmp(action,'levcomp')
  
    dflevel = gcf;
    ud = get(dflevel,'UserData');
    dfdisp = findobj('name','dfield8b Display');
    dud = get(dfdisp,'UserData');
    dfset = findobj('name','dfield8b Setup');
    sud = get(dfset,'UserData');
	...
	if isempty(pv) 
	  ...
	  pv(find(abs(pv)==32))=[];
	  lfcn = dfield8b('paraeval',pn,pv,lfcn);
	end
      end
    end
    l = length(lfcn);
  ...
  elseif strcmp(action,'showbar')
    ...
    if strcmp(state,'figure')
	  ...
	  'clickedcallback',['dfield8b(''zoomback'');' fixtb]);
      set(findobj(sbfig,'tooltipstr','Zoom In'),...
	  'clickedcallback',['dfield8b(''zoomin'');' fixtb]);
      set(findobj(sbfig,'tooltipstr','Print'),...
	  'clickedcallback','dfield8b(''print'');');
  ...
  elseif strcmp(action,'figdefault')
    ...
    fig = input1;
    set(fig,'CloseRequestFcn','dfield8b(''closefcn'')');
    dfset = findobj('name','dfield8b Setup');
    sud = get(dfset,'UserData');
    ud = get(fig,'UserData');
    ud.ssize = sud.ssize;
  ...
  elseif strcmp(action,'closefcn')
    ...
    fig = gcf;  
    name = get(fig,'name');
    if strcmp(name,'dfield8b Setup') | strcmp(name,'dfield8b Display')
      quest = ['Closing this window will cause all dfield8b ',...
	       'windows to close, and dfield8b will stop.  ',...
	       'Do you want to quit dfield8b?'];
      butt = questdlg(quest,'Quit dfield8b?','Quit','Cancel','Quit');
      if strcmp(butt,'Quit')
	dfield8b('quit');
      end
    elseif strcmp(name,'dfield8b Linearization')
  ...
  elseif strcmp(action,'confused')
    ...
    tstring = 'dfield8b is totally confused';
    qstring = {['You will have to restart dfield8b from '...
		...
		'it would be best to do nothing now.'];
	       'What do you want to do?'};
    bstr1 = 'Quit and restart dfield8b.';
    bstr2 = 'Just quit dfield8b.';
    bstr3 = 'Do nothing.';
    answer = questdlg(qstring,tstring,bstr1,bstr2,bstr3,bstr1);
    if strcmp(answer,bstr1)
      delete(findobj('tag','dfield8b'));
      dfield8b;return
  ...
  elseif strcmp(action,'loadsyst')  % This loads either a system or a gallery.
	...
	errmsg = 'This is not a dfield8b file.';
	fprintf('\a')
	errordlg(errmsg,'dfield8b error','on');
	return
      end
    ...
    for j = 1:nnn
      ...
      newsyst.name = sname;
      ignore = dfield8b('addgall',newsyst);
      if ignore == -1;
